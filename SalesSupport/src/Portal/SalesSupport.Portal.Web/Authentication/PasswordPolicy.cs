...
public sealed class PasswordPolicy : IPasswordPolicy
{
    ...
    /// <param name="path">Web公開領域外に配置した1行1件のUTF-8テキストの絶対パスです。</param>
    ...
    {
        if (string.IsNullOrWhiteSpace(path) || !Path.IsPathFullyQualified(path)) throw new ConfigurationException("SalesSupport:Password:ForbiddenListPath");
        if (IsInside(path, webRootPath)) throw new ConfigurationException("SalesSupport:Password:ForbiddenListPath");
        string[] lines;
