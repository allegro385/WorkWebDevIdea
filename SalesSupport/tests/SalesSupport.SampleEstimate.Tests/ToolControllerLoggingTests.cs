...
using SalesSupport.SampleEstimate.Web.Controllers;
...
public sealed class ToolControllerLoggingTests
{
    ...
    /// <summary>保存時は案件を登録して詳細GETへ移り、実行成功を一度だけ記録します。</summary>
    [Fact]
    public async Task ExecuteSaveRedirectsAfterRecordCreation()
    {
        var usage = new RecordingUsageLogger();
        var records = new StubRecordService();
        var controller = CreateController(new StubEstimateService(), usage, records);
        var submissionId = Guid.NewGuid();

        var response = await controller.Execute(new EstimateInput
        {
            Output = EstimateOutput.Save, SubmissionId = submissionId, CategoryCode = "STANDARD"
        }, null, CancellationToken.None);

        var redirect = Assert.IsType<RedirectToActionResult>(response);
        Assert.Equal("Details", redirect.ActionName);
        Assert.Equal("Records", redirect.ControllerName);
        Assert.Equal(submissionId, records.SubmissionId);
        Assert.Equal("SUCCESS", Assert.Single(usage.Events).ResultCode);
    }

    /// <summary>送信IDのない保存要求はDBへ書かず、失敗として一度記録します。</summary>
    [Fact]
    public async Task ExecuteSaveRejectsMissingSubmissionId()
    {
        var usage = new RecordingUsageLogger();
        var records = new StubRecordService();
        var controller = CreateController(new StubEstimateService(), usage, records);

        var response = await controller.Execute(new EstimateInput { Output = EstimateOutput.Save }, null, CancellationToken.None);

        Assert.IsType<ViewResult>(response);
        Assert.Null(records.SubmissionId);
        Assert.Equal("FAILURE", Assert.Single(usage.Events).ResultCode);
    }

    /// <summary>Serviceの項目エラーを入力欄の`Input.`付きキーへ結び付けます。</summary>
    [Fact]
    public async Task ExecuteDisplaysServiceErrorsAtInputFields()
    {
        var usage = new RecordingUsageLogger();
        var controller = CreateController(new StubEstimateService { InvalidResult = true }, usage);

        var response = await controller.Execute(new EstimateInput(), null, CancellationToken.None);

        Assert.IsType<ViewResult>(response);
        Assert.True(controller.ModelState.ContainsKey("Input.Quantity"));
        Assert.Equal("FAILURE", Assert.Single(usage.Events).ResultCode);
    }
    ...
    {
        var usage = new RecordingUsageLogger();
        var filter = new WebOpenLoggingFilter(usage, Options.Create(new CommonOptions { ToolId = "SAMPLE-ESTIMATE" }));
    ...
    private static ToolController CreateController(IEstimateService estimates, IUsageLogger usage, StubRecordService? records = null) => new(
        estimates, records ?? new StubRecordService(), new StubToolStatusService(), usage, new StubBusinessDateProvider(),
        Options.Create(new CommonOptions { ToolId = "SAMPLE-ESTIMATE" }))
    {
        ControllerContext = new ControllerContext { HttpContext = new DefaultHttpContext() }
    };

    /// <summary>DBを使わず公開状態の表示名を返します。</summary>
    private sealed class StubToolStatusService : IToolStatusService
    {
        /// <summary>固定の表示名を返します。</summary>
        public Task<string> GetStatusNameAsync(CancellationToken ct = default) => Task.FromResult("限定公開");
    }

    /// <summary>ログ検証ではDBを使わないため、保存要求だけを受け取る代替です。</summary>
    private sealed class StubRecordService : IEstimateRecordService
    {
        public Guid? SubmissionId { get; private set; }
        /// <summary>空の一覧を返します。</summary>
        public Task<EstimateRecordList> ListAsync(DateOnly? from, DateOnly? to, string? categoryCode, CancellationToken ct = default) =>
            Task.FromResult(new EstimateRecordList([], 0, 0));

        /// <summary>保存済み案件がない状態を返します。</summary>
        public Task<EstimateRecord?> GetAsync(Guid recordId, CancellationToken ct = default) => Task.FromResult<EstimateRecord?>(null);

        /// <summary>固定IDの登録結果を返します。</summary>
        public Task<Guid> CreateAsync(Guid submissionId, EstimateInput input, EstimateResult result, CancellationToken ct = default)
        {
            SubmissionId = submissionId;
            return Task.FromResult(Guid.Parse("00000000-0000-0000-0000-000000000001"));
        }

        /// <summary>更新が成功した結果を返します。</summary>
        public Task<RecordUpdateResult> UpdateAsync(Guid recordId, int updateCount, EstimateInput input, EstimateResult result, CancellationToken ct = default) =>
            Task.FromResult(RecordUpdateResult.Updated);
    }
    ...
    private sealed class StubEstimateService : IEstimateService
    {
        ...
        public bool InvalidResult { get; init; }
        ...
        public Task<EstimateOutcome> CalculateAsync(EstimateRequest request, CancellationToken ct = default) =>
            Task.FromResult(InvalidResult
                ? new EstimateOutcome(EstimateOutcomeKind.InvalidInput, null,
                    new ValidationResult([new FieldError(nameof(EstimateInput.Quantity), "OUT_OF_RANGE", "数量が不正です。")]))
                : new EstimateOutcome(EstimateOutcomeKind.Calculated,
